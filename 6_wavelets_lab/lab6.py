...
def non_uniform_map(img):
    ...
    output = np.copy(img)    
    # Avoid the case in which the minimum of the image is below 0
    # (in which case the negative mapping is not necessary, as it would not be applied anywhere)
    img_min = np.min(img)
    if img_min < 0:
        # Min maps to 0, 0 maps to 127.5
        output[img<0] = 127.5 - 127.5*img[img<0]/img_min
    # Avoid the case in which the maximum of the image is above zero
